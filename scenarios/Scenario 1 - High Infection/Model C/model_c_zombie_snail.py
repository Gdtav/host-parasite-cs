...
def initialize():
    ...
    poop = np.array([np.count_nonzero(config == 11)])
...
def update():
        ...
        for y in range(height):
            ...
            if state == 10 and nextConfig[x, y] != 10:
                ...
                for neighbour in neigh_list:
                    if config[neighbour[0], neighbour[1]] == 11 and nextConfig[neighbour[0], neighbour[1]] == 11:  # poop in vicinity and no snail ate it
                ...
                if random() < deathProb:
                    ...
                    nextConfig[x_, y_] = 11 # poop in this cell
    ...
    ## Poop - State 3
    number_poop_cells = np.count_nonzero(config == 11)
