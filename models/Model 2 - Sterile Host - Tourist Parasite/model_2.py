...
transmissionRate = 0.10  # Probability of moving body
...
def update():
    ...
    for x in range(width):
        for y in range(height):
            state = config[x, y]
            if state == 2:
                if random() < transmissionRate: # tourism mode activated
                    state = 0  # empty
                    neigh_list = neighbours(x, y)
                    for neighbour in neigh_list:
                        if config[neighbour[0], neighbour[1]] == 1:
                            print(f'Parasite in {x}, {y} moved to {neighbour[0]}, {neighbour[1]}\n')
